...
export function SpotlightCard({
  ...
  const isMyGroup =
    ...
    allPlayers.some((p) => p?.discordId === currentPlayerId);

  // Create an array of non-null players for the portraits
  const groupPlayers = [group.tank, group.healer, ...group.dps].filter(
    (p): p is WoWPlayer => p !== null,
  );

  return (
    <div
      className={`spotlight-wrapper${visible ? " spotlight-visible" : ""}${exit ? " spotlight-exit" : ""}`}
    >
      <div className={`spotlight-card${isMyGroup ? " is-my-group" : ""}`}>
        <h3 className="spotlight-heading">
          {label ??
            (isMyGroup
              ? `Group ${index + 1} — Your Group!`
              : `Group ${index + 1}`)}
        </h3>
        {group.tank && (
          <SpotlightRoleRow
      ...
      <SpotlightPortraits players={groupPlayers} />
    </div>
  );
}

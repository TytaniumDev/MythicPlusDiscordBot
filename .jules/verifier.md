...
## 2026-05-11 - Mocking Complex Discord.js Cache Collections
**Challenge:** Testing functions like `adaptGuild` that interact with deeply nested Discord.js structures, such as `guild.channels.cache` mapping and filtering methods.
**Solution:** Construct a structured mock object that overrides the array manipulation methods directly (e.g., overriding `cache.filter` to return a raw array and `cache.map` to operate on `this.filter()`). When using functions like `get`, utilize a dummy switch/if block to simulate basic lookups. Use type assertions (`as any`) to bypass exact typings for deeply nested elements where minimal stubs suffice.
**Guideline:** When testing logic that relies on `discord.js` Collections, do not attempt to mock the entire class or instantiate real Discord.js classes; instead, stub only the exact iteration/lookup methods (like `filter`, `map`, `get`) utilized by the implementation under test.

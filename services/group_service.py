...
import os
from config import (
    SPIN_SOUND, REVEAL_SOUND, WHEEL_GIF, PLACEHOLDER_CHAR
)
from core.parallel_group_creator import create_mythic_plus_groups
from utils.data_helpers import getPlayerList
from utils.discord_helpers import join_voice_channel, play_sound, dashed, showShortTyping
...
class GroupService:
    ...
    async def coreWheel(self, ctx, debugValue: bool = None, enhanced: bool = False):
        ...
        async with server_lock:
            await self._execute_coreWheel(ctx, channel, guild_id, debug, enhanced)

    async def _execute_coreWheel(self, ctx, channel, guild_id, debug, enhanced=False):
        ...
        if debug:
            # Testing Code - trying to find a specific channel for testing if needed
            # For now, just use the current channel logic as fallback or assume context is correct
            # Original code: testChannel = discord.utils.get(ctx.guild.channels, name='path-of-exile')
            # Keeping it simple and using current channel members for now unless strictly needed

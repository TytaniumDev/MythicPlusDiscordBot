...
## 2026-04-27 - Mocking Missing Context Properties
**Challenge:** When testing error paths in command handlers (e.g., `GroupsHandler.wheel`), deleting required properties from the default mock context object bypasses TypeScript compile errors but can be missed during coverage review if the path throws before reaching expected functionality.
**Solution:** Used the standard `makeCtx()` factory function and dynamically removed the property (`delete ctx.channel`) specifically inside the test to simulate unexpected caller state while preserving standard mock interfaces.
**Guideline:** When testing missing required context fields (like `ctx.channel`), dynamically delete the field from a fully-constructed mock object rather than building an incomplete mock to ensure realistic simulation of runtime missing data without breaking the broader test suite factory.

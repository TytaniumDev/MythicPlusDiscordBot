...
def generate_wheel_gif(filename: str) -> None:
    ...
    for i in range(steps):
        ...
        draw.polygon(
            [(width // 2 - 10, 5), (width // 2 + 10, 5), (width // 2, 25)], fill="black"
        )
    ...
    images[0].save(
        filename, save_all=True, append_images=images[1:], duration=80, loop=0
    )
...
if __name__ == "__main__":
    ...
    download_file(
        "https://actions.google.com/sounds/v1/tools_and_machines/mechanical_clock_ticking.ogg",
        os.path.join(assets_dir, "spin.ogg"),
    )
    download_file(
        "https://actions.google.com/sounds/v1/notifications/pizzicato_confirm.ogg",
        os.path.join(assets_dir, "reveal.ogg"),
    )

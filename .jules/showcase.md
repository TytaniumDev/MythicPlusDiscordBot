...
- **[2026-04-09]** Updated `README.md` to use `./setup.sh` instead of `npm ci` for the "Quick Start" command. The previous `npm ci` resulted in missing system dependencies (`ffmpeg` and `libnacl-dev`) and caused bot features to fail without clear errors, leading to UX friction during initial onboarding. Also updated the configuration section to ensure the `.env` file is accurately placed directly into `packages/bot/.env`.

...
def create_mythic_plus_groups(players: List[WoWPlayer]) -> List[WoWGroup]:
    ...
    print(f'Available tanks (main + off): {len(available_tanks)}')
    ...
    print(f'Available healers (main + off): {len(available_healers)}')
    ...
    print(f'Available DPS (main + off): {len(available_dps)}')
    ...
    print(f'Maximum possible complete groups: {max_possible_groups}')
    ...
    while True:
        ...
        print(f'--- Handling {len(remaining_players)} remaining players ---')
        ...
        if new_group:
            ...
            print(f'Added {"complete" if new_group.is_complete else "partial"} group with {new_group.size} players')
                    ...
                    if new_group:
                        for player in [new_group.tank, new_group.healer, new_group.dps1, new_group.dps2, new_group.dps3]:
                            if player:
                                removePlayer(player)
                        groups.append(new_group)
                        print(f'Added extra partial group with {new_group.size} players')
                break

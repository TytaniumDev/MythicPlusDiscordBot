...
## 2026-08-15 - [SSRF/Path Traversal in Battle.net API Calls]
**Vulnerability:** Constructing Battle.net API URLs using unsanitized variables (like `realmSlug` and `characterName`) allowed for potential path traversal or SSRF vulnerabilities.
**Learning:** Even internal API calls constructed from strings must have their URL components properly encoded, especially if any part of the string might originate from client-writable data sources.
**Prevention:** Always use `encodeURIComponent` when interpolating dynamic data into URL paths or query strings.

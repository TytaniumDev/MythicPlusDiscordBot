...
const lobbyIdentity = {
  id: mockPlayers[4].discordId,
  name: mockPlayers[4].name,
};
  ...
  test.describe(`${name} (${viewport.width}x${viewport.height})`, () => {
    ...
    test("Channel Picker", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        `channels-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("Lobby", async ({ page }) => {
      ...
      await expect(page.locator(".player-chip")).toHaveCount(
        mockPlayers.length,
      );
    ...
    test("Lobby Sitting Out", async ({ page }) => {
      ...
      await expect(page.locator("#view-lobby")).toBeVisible();
      await expect(
        page.locator(".role-section-header--sitting-out"),
      ).toBeVisible();
      await expect(page.locator(".player-chip.sitting-out")).toHaveCount(2);
      await expect(page).toHaveScreenshot(
        `lobby-sitting-out-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("Lobby Empty", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        `lobby-empty-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("Static Wheels", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        `wheels-static-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("Spinning Ready", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        `spinning-ready-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("Results", async ({ page }) => {
      ...
      await expect(page.locator("#final-groups .group-card")).toHaveCount(
        mockGroups.length,
      );
      ...
      await expect(page).toHaveScreenshot(
        `results-${viewport.width}x${viewport.height}.png`,
      );
    });
    ...
    test("No Session", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        `no-session-${viewport.width}x${viewport.height}.png`,
      );
    });
  });
}
...
test.describe("Carousel Mode Tests", () => {
  test.describe("Phone Portrait (420x700)", () => {
    ...
    test("Carousel navigation via dot click scrolls to wheel", async ({
      page,
    }) => {
      await page.goto(`/?data=${encodeData(staticWheelsData)}`);
      await expect(page.locator("#view-wheels")).toBeVisible();
  ...
  test.describe("Discord Square (380x380)", () => {
    ...
    test("Side panel hidden on mobile, replaced by group pager", async ({
      page,
    }) => {
      await page.goto(`/?data=${encodeData(staticWheelsData)}`);
...
test.describe("Functional Tests", () => {
  ...
  test("Results show correct group composition", async ({ page }) => {
    ...
    await expect(group1.locator(".role-name").first()).toHaveText(
      "Pandemonium ⚰️",
    );
...
test.describe("Home View Tests", () => {
  ...
  test("Shows home view with recent guilds from localStorage", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".guild-card-name").first()).toHaveText(
      "Gif or Gif",
    );
    await expect(page.locator(".guild-card-name").nth(1)).toHaveText(
      "Another Guild",
    );
  });
...
test.describe("Spotlight Card Visual Tests", () => {
  test.use({ viewport: { width: 1280, height: 720 } });

  test("Spotlight Card with Portraits Desktop", async ({ page }) => {
    await page.goto(
      "http://localhost:6006/iframe.html?id=components-spotlightportraits--complete-group&viewMode=story",
    );
    await expect(page.locator(".spotlight-portraits")).toBeVisible();
  });
});

test.describe("Spotlight Card Visual Tests Mobile", () => {
  test.use({ viewport: { width: 380, height: 720 } });

  test("Spotlight Card with Portraits Mobile Stack", async ({ page }) => {
    await page.goto(
      "http://localhost:6006/iframe.html?id=components-spotlightportraits--complete-group&viewMode=story",
    );
    await expect(page.locator(".spotlight-portraits")).toBeVisible();
